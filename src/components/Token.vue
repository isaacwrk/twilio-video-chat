<template>
    <div class="text-center justify-center pt-2">
        ...
        <button class="bg-red-200 text-red-500 w-16 h-16 rounded-md text-xs" @click="getAcessToken">Recuperar Token</button>
...
const Token = defineComponent({
    ...
    setup(){
        ...
        async function getAcessToken(){
            const result  = await axios.get(`http://localhost:3000/token?identity=${nome.value}`);
